<?php
	include("session.php");
	include("config.php");
	$id = $_GET['id'];
?>
<!DOCTYPE html>
<html>
<head>
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<link rel="stylesheet" type="text/css" href="css/mystyle1.css" /> 
</head>
<body>
<div class="icon-bar">
  <a href="home.php"><i class="fa fa-home"></i></a> 
  <a href="students.php"><i class="fa fa-user"></i></a> 
  <a class="active" href="registration.php"><i class="fa fa-registered"></i></a>
  <a href="logout.php"><i class="fa fa-power-off"></i></a> 
</div>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
<h2>Update Students</h2>
<hr/>

<form action="update_students.php" method="POST">
  <div class="container">
  <?php
	$result = mysqli_query($mysqli,"SELECT * FROM students WHERE Reg_No ='$id'");
	while($row = mysqli_fetch_array($result))
	{
	echo"<input type='hidden' name='id' value='{$row['Reg_No']}' required>";
    echo"<input type='text' placeholder='Reg_No' name='Reg_No' value='{$row['Reg_No']}' required>";
    echo"<input type='text' placeholder='First name' name='FirstName' value='{$row['FirstName']}' required>";
    echo"<input type='text' placeholder='Last name' name='LastName' value='{$row['LastName']}' required>";
  	echo"<label><b>Birthday</b>";
    echo"<input type='date' name='DOB' value='{$row['DOB']}'required>";
    echo"</label>";
    echo"<input type='text' placeholder='Gender' name='Gender' value='{$row['Gender']}' required>";
    echo"<input type='text' placeholder='Phone' name='Phone' value='{$row['Phone']}' required>";
    echo"<input type='text' placeholder='Email' name='Email' value='{$row['Email']}' required>";
    echo"<input type='text' placeholder='Residence' name='Residence' value='{$row['Residence']}' required>";
    echo"<input type='text' placeholder='Course' name='Course' value='{$row['Course']}' required>";
    //echo"<input type='text' placeholder='Username' name='username' value='{$row['username']}' required>";
    echo"<input type='password' placeholder='Password' name='Password' value='{$row['Password']}' required>";
    echo"<div class='clearfix'>";
    echo"<button type='submit' class='signupbtn'>Update</button>";
	echo"</div>";
	}?>
  </div>
</form>